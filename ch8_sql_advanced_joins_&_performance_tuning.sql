...
Some high-level things that will affect the number of calculations a given query will make include:
  ...
  2. Joins 
    - if your query joins two tables in a way that substantially increases the row count of the result set, youre query is likely to be slow.
  3. Aggregations 
    - requiring multiple rows to produce a result requires more computation than simply retrieving those rows. 
...
2. Database software and optimization 
  - different databases vary in speed for a given task 
...
Lists the primary point of contact and sales rep for each account, only where the sales primary point of contact's full 
